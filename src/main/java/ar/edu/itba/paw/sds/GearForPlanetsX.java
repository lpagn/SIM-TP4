...
public class GearForPlanetsX {
    ...
    public double a(double x, double noSirve) {
        return planet.frx(influence, x).x / planet.mass;
    }


    private static double[] alfalist = {3.0 / 16, 251.0 / 360, 1, 11.0 / 18, 1.0 / 6, 1.0 / 60};

    public GearForPlanetsX(double step, Planet planet, Planet[] influence) {
        this.step = step;
        this.planet = planet;
        this.influence = influence;
        r = new ArrayList<>();
        r.add(planet.position.x);
        r.add(planet.v.x);
