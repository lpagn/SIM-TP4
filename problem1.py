...
plt.axis([0,5,-1,1]) # Use the plt.axis() function with your limits. | where x(y)min/max are the coordinate limits for both axes.
...
plt.loglog(gear_register_x,gear_register_y)    
#plt.show()
...
plt.xlabel(' step  [s]  ')
plt.ylabel(' error [m*m]')
plt.loglog(beeman_register_x,beeman_register_y)    
#plt.show()
...
plt.xlabel(' step  [s]  ')
plt.ylabel(' error [m*m]')
plt.loglog(leapfrog_register_x,leapfrog_register_y)    
#plt.show()
